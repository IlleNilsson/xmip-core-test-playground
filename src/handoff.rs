//! The handoff between node processes: a pair's bytes passed `R` to `P` to
//! `S` through per-node inboxes in the cluster's shared directory.
//!
...
//! them. This is what goes between them. Each node has one [`Inbox`],
//! `<shared>/handoff/<node>/`. A sender writes a [`Handoff`] under a temporary
...
pub struct Handoff {
    ...
    /// The round the `R` node received it in: the pair's id with the two
...
impl Inbox {
    /// The inbox of `node` under `shared`: `<shared>/handoff/<node>/`.
    #[must_use]
    pub fn of(shared: &Path, node: &str) -> Self {
        Self {
            dir: shared.join("handoff").join(node),
        }
    }
...
/// The hops over one link: how many handoffs `from` delivered `to`, and when
/// the last one was.
...
impl Hops {
    /// Record one delivered handoff `from` one node `to` another.
    pub fn record(&mut self, from: &str, to: &str, now: i64) {
        let hop = self
            .links
...
mod tests {
    ...
    fn a_half_written_file_is_never_claimed_and_a_torn_one_is_counted() {
        ...
        let inbox = Inbox::of(&shared, "S1");
        inbox.deliver(&handoff(1, b"whole"), 1).expect("delivered");
        let dir = shared.join("handoff/S1");
    ...
    #[test]
    fn hops_count_per_link_and_keep_the_last_time() {
